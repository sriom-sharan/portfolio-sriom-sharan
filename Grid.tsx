          ...
          {skillsFrontend.map(({ title, img, id }) => (
            <Fragment key={id}>
              <div className="flex items-center md:max-w-60 max-w-full gap-2">
                <Image
                  src={img.startsWith('http') ? img : `${basePath}${img}`}
                  alt={title}
        ...
        {gridItems.map(
          (
            ...
            <BentoGridItem
              ...
              className={className}
              img={img ? (img.startsWith('http') ? img : `${basePath}${img}`) : img}
              imgClassName={imgClassName}
              titleClassName={titleClassName}
              spareImg={spareImg ? (spareImg.startsWith('http') ? spareImg : `${basePath}${spareImg}`) : spareImg}
            />
          )
        )}
      </BentoGrid>
